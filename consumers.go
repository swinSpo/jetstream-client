...
var DefaultConsumerOpts = []ConsumerOption{
	DeliverAllAvailable(),
	AcknowledgeExplicit(),
...
// DefaultConsumer is the configuration that will be used to create new Consumers in NewConsumer
var SampledDefaultConsumerOpts = []ConsumerOption{
	DeliverAllAvailable(),
	AcknowledgeExplicit(),
	AckWait(time.Second * 30),
	ReplayInstantly(),
...
func NewConsumer(stream string, opts ...ConsumerOption) (consumer *Consumer, err error) {
	options := MergeConsumerOptions(DefaultConsumerOpts, opts...)
...
func LoadOrNewConsumer(stream string, name string, opts ...ConsumerOption) (consumer *Consumer, err error) {
	...
	if c == nil || err != nil {
		return NewConsumer(stream, opts...)
	}
...
// MergeConsumerOptions generates a new configuration based on template modified by opts
func MergeConsumerOptions(baseOptions []ConsumerOption, opts ...ConsumerOption) ([]ConsumerOption) {
	return append(baseOptions, opts...)
}
...
func WithDeliverySubject(s string) ConsumerOption {
