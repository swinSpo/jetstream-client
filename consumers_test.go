...
func TestNew(t *testing.T) {
	...
	//simple case. Uses DefaultConsumerOpts as default
	consumer, err := jsch.NewConsumer("streamName", jsch.AcknowledgeAll())

	//sophisticated case with a base "template"
	baseTemplate := []jsch.ConsumerOption{
		jsch.AcknowledgeAll(),
		jsch.AckWait(time.Minute),
	}
	opts := jsch.MergeConsumerOptions(baseTemplate, jsch.AcknowledgeAll())
	consumer, err := jsch.NewConsumer("streamName", opts...)

	//simple LoadOrNewConsumer
	consumer, err := jsch.LoadOrNewConsumer("streamName", "bookdurable", jsch.AcknowledgeAll(), jsch.DurableName("bookdurable"))

	//sophisticated LoadOrNewConsumer
	opts := jsch.MergeConsumerOptions(baseTemplate, jsch.AcknowledgeAll(), jsch.DurableName("bookdurable"))
	consumer, err := jsch.LoadOrNewConsumer("streamName", "bookdurable", opts...)
	...
	jsch.DefaultConsumerOpts


	opts := jsch.MergeConsumerOptions(jsch.DefaultConsumerOpts, jsch.AcknowledgeAll())
	consumer, err := jsch.NewConsumer("streamName", opts...)
